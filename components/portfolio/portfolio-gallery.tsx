...
export function PortfolioGallery({ filter, items }: PortfolioGalleryProps) {
  ...
  return (
    <>
      <section
        ...
        className="relative bg-[#0a1220] py-16 text-white lg:py-20"
      >
        ...
        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <p className="mb-10 text-xs font-semibold uppercase tracking-[0.35em] text-white/40">
            Grid view · {filtered.length} project{filtered.length === 1 ? "" : "s"}
          </p>
                  ...
                  <div className="group relative w-full overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] text-left backdrop-blur-sm transition hover:border-[var(--color-accent)]/40">
                    <div className="relative aspect-[4/5] w-full overflow-hidden sm:aspect-[3/4]">
                      ...
                      <div className="absolute right-3 top-3 z-20 flex gap-2">
                        {getPortfolioLiveUrl(item) ? (
                          <a
                            href={getPortfolioLiveUrl(item)}
                            target="_blank"
                ...
                <div className="flex flex-wrap gap-3">
                  {getPortfolioLiveUrl(active) ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--color-accent)]/40 bg-[var(--color-accent)]/10 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-[var(--color-accent)] hover:text-[var(--color-nav)]"
                    >
                      View live
                    </a>
                  ) : null}
